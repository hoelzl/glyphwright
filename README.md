...
The session accepts `move <north|east|south|west>`, `look`, `wait`, `take <item>`, `use <item>`, `equip <item>`, `attack <target>`, `flee` (in battle), `help`, and `quit`. Each turn prints a transcript block anchored by `== turn N · mode · area ==`. The frame's command grammar always names exactly what is valid right now.
