"""Keystroke translation shared by the interactive frontends.
...
so the keyboard can never say something the grammar cannot (0003 §6). The
TUI and the GUI both consume this table, so the two interactive frontends
cannot drift apart on bindings (0011 §4). Keys exist only in frontends
(ADR-003); how raw input becomes these key names is each frontend's own
decoding concern.
"""
