...
| 3 | **Battle.** Menu battle first, forcing the mode stack and shared initiative scheduler into existence; tactics arena reusing `GridSpace` as a follow-up. | In progress — 3A (exploration combat + shared scheduler) done; 3B (menu battle mode) next |
